...
void handleDetailsPage(HTTPRequest * req, HTTPResponse * res) {
  ...
  res->println("# End Session 1\nThis is definitely possible. I reckon another hour or two of work and it'll be good to go. Time to go hang up some washing, buy some vegetables and get on with my Saturday morning (ahem, afternoon)\n");
  res->println("## Where am I going with this?\n\n* Serve static gemtext from the ESP's flash\n* Serve a dynamic page from the server (uptime, status etc.)\n* Serve pages from a SD card.\n\nMy current plan is to load content onto my server by ... switching SD cards out! Maybe in future I'll give it some other interface?\n\nAlso, maybe I could make this server solar powered. An ESP32 draws ~1W when active (300mA, 3.3v), so I need maybe ~20Wh of battery and 10W of panel. That's a moderate USB battery bank and a pretty small panel.\n");
  ...
  res->println("# End Session 2\nThat'll do me for now. Let me set up port forwarding and make it live. Let's also see if I can serve this bunch of scribblings as a page!\n");
